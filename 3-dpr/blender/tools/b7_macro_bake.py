...
"""b7_macro_bake.py -- the per-mesh MACRO layer. Runs INSIDE Blender.

Bakes the large-scale, world-Z-driven terms that a tiling surface texture
cannot hold -- stratification, water stains and the wet band -- plus AO, to
one 1K map per mesh on UVMap. Multiplied over the tiling set at render time.

The node chain here is NOT invented: it is lifted verbatim from the surviving
procedural look-dev in cave_B5_procedural.blend, whose MACRO_* labelled nodes
split cleanly from its TILE_* nodes (the TILE half is already baked into
textures/T_Limestone_*.png by b5_textures.py).

Packing, one map per mesh, Non-Color:
    R = albedo multiplier / RATIO_SCALE   (strata + stains + wet band)
    G = ambient occlusion                 (Godot reads this; Principled has no AO input)
    B = roughness, absolute 0.15 - 0.82   (the wet band lives here)
    A = 1

Idempotent: re-running overwrites the same files and leaves no temp datablocks.
"""
import bpy, os, time
import numpy as np

SIZE        = 1024
RATIO_SCALE = 1.5     # macro/base ratio peaks at ~1.33; see verify() output
AO_DISTANCE = 3.0     # finite, or a sealed chamber bakes to near-black (handoff section 6)
SAMPLES     = 32
MARGIN      = 8

# strata overlay factor, stain gain -- per material, straight off the procedural trees
TARGETS = {
...
def build_macro(nt, strata_f, stain_gain):
    ...
    # MACRO_stains -- Z stretched into vertical drips, gated to strengthen near the floor
    ...
    # roughness: dry 0.82 -> stained 0.62 -> wet 0.15
...
def bake_one(obj_name, strata_f, stain_gain, texdir):
    ...
    cov = base[..., 0] > 1e-4                      # texels the bake actually touched
    ...
    # unbaked background -> neutral, so the margin can never darken a seam
...
def run():
    ...
    # <project>/blender/cave.blend -> <project>/textures
